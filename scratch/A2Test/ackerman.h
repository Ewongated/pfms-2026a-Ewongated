 ...
 * Uses the Audi kinematic model to compute steering. Throttle ramps down over
 * the first half of each leg. Braking is velocity-based — full brake is applied
 * when the current stopping distance exceeds the distance to the goal, ensuring
 * consistent behaviour regardless of journey length or speed. When a goal is
 * unreachable via constant steering, brakes to a stop then drives a tight
 * full-lock arc at low speed until reachable.
 */
